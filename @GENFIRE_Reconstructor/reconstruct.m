...
if obj.monitor_R == 1
    obj.errR = zeros(1,floor(numIterations./obj.monitorR_loopLength));
    obj.Rarr_record = zeros(obj.NumProjs,floor(numIterations./obj.monitorR_loopLength));
    obj.Rarr2_record = zeros(obj.NumProjs,floor(numIterations./obj.monitorR_loopLength));
end
for iterationNum = 1:numIterations
    ...
    % R-factor
    if obj.monitor_R == 1 && mod(iterationNum,obj.monitorR_loopLength) == 0
        Rarr = zeros(obj.NumProjs,1);
        Rarr2 = zeros(obj.NumProjs,1);
        ...
        errR = mean(Rarr);
        errR2 = mean(Rarr2);
        fprintf('Iteration %d. Rfactor=%.4f, R2factor=%.4f \n',iterationNum, errR, errR2);
        obj.errR(iterationNum./obj.monitorR_loopLength) = errR;
        obj.Rarr_record(:,iterationNum./obj.monitorR_loopLength) = Rarr;
        obj.Rarr2_record(:,iterationNum./obj.monitorR_loopLength) = Rarr2;
    end
end
