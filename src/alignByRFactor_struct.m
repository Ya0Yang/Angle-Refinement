...
function [R, new_x_center, new_y_center] = alignByRFactor_struct(ref_img,img,window_half_size)
...
%         R = sum(sum(abs(ref_img-circshift(img,[sx, sy])))) ./ norm_factor;
        ops.error_tol = 1e-5;
        R = compute_struc(abs(circshift(img, [sx, sy]) - ref_img), ops)./ norm_factor;
        if R < bestR
           bestR = R;
           new_x_center = ncx - sx;
           new_y_center = ncy - sy;
