...
function obj = refineSpatialAlignBGopt_R_parallel(obj)
...
parfor pj_num = 1:num_proj
    ...
    if Rres < 1
        tempPP = real(My_FourierShift(refineFullProjs(:,:,pj_num)+BGforAdd*(refineFullProjs(:,:,pj_num)~=0),-1*d1*bin_factor, -1*d2*bin_factor));
        tempPP(tempPP<0) = 0;
        realignedProjs(:,:,pj_num) = tempPP;
    else
        tempPP = circshift(refineFullProjs(:,:,pj_num)+BGforAdd*(refineFullProjs(:,:,pj_num)~=0),[-1*d1*bin_factor, -1*d2*bin_factor]);
        tempPP(tempPP<0) = 0;
        realignedProjs(:,:,pj_num) = tempPP;
    end
